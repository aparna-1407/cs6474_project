...
### Overall Pipeline
<p align="center">
  ![image](https://github.com/aparna-1407/cs6474_project/assets/93538009/34d0a200-aec0-4246-bbf9-35d6417490f4)
</p>
...
2.  <p style="font-weight:bold">Appearance-Disentangled Pose Control Module</p><p> involves finetuning the Stable Diffusion ControlNet pipeline to use the ACM instead of just stable diffusion. This finetuning helps to disentangle pose retargeting from the appearance. We basically have to finetune these modules with the a pairs of source image-pose and target image. Using a dataset where images with multiple poses of the same person are available will be used.</p>
3. <p style="font-weight:bold">Text Guided Enhancement Module</p><p>which consists of a text encoder which creates prompt embeddings and we combine this prompt embeddings used with subject/source embeddings (understanding the structure of the source) generated by the InstructPix2Pix model within stable diffusion. The cross attention layers of stable diffusion's cross attention layers uses the embeddings and masks regions to be editted and the prompt embeddings guides the editting. This will leverage the prior knowledge of stable diffusion and not require any finetuning.</p>

> For Project Update-1 we have completed part of the Appearance Disentangled Pose Control Module. We have identified how to set up the pipeline corrently and how ControlNet works best with Stable Diffusion to generate the right output. More details on the different approaches attempted will be explained in next sections.
>
